...
exports.validate_user_signup = [
    check('f_name')
        ...
        .isString().withMessage("Invalid first name, must be string")
        .trim()
        .isLength({min: 3, max: 30}).withMessage('First name must be between 3 and 20 characters!'),
    check('l_name')
        .notEmpty().withMessage('Last name required!')
        .isString().withMessage("Invalid last name, must be string")
        .trim()
        .isLength({min: 3, max: 30}).withMessage('Job name must be between 3 and 20 characters'),
    check('email')
    ...
    check('password')
        ...
        .isString().withMessage("Invalid Password, must be string")
        .trim()
    ...
    check('confirm_password')
        ...
        .isString().withMessage("Invalid confirmation password, must be string")
        .trim()
        .isLength({min: 8, max: 30}).withMessage('Confirmation password must be between 8 and 30 characters')
        .custom((value, {req}) => {
    ...
    check('phone')
        .optional()
        .isString().withMessage("Invalid phone number, must be string")
        .trim().isLength({min: 11, max: 13}).withMessage('Phone number must be 11 or 13 numbers'),

    check('gender')
        .optional({nullable: true})
        .trim()
        .isString().withMessage("Invalid gender, must be string")
        .isIn(['male', 'female']).withMessage('gender must be male or female'),

    check('age')
        .optional({nullable: true})
        .notEmpty()
        .isString().withMessage("Invalid age, must be string")
        .isLength({min: 1, max: 3}).withMessage("Age must be a valid number"),

    check('city')
        .optional({nullable: true})
        .notEmpty()
        .isString().withMessage("Invalid city, must be string")
        .trim().isLength({min: 3, max: 20}).withMessage('city must be between 3 and 20 characters'),
    check('country')
        .optional({nullable: true})
        .notEmpty()
        .isString().withMessage("Invalid country, must be string")
    ...
    check('skills')
        .optional({nullable: true})
        .isArray().withMessage('Skills must be an array of string ["skill 1","skill 2"]'),
]
